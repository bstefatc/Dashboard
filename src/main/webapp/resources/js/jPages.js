/**
 * jQuery jPages v0.7 Client side pagination with jQuery http://luis-almeida.github.com/jPages
 * 
 * Licensed under the MIT license. Copyright 2012 Luís Almeida https://github.com/luis-almeida
 */
...
(function($, window, document, undefined) {
	...
	Plugin.prototype = {
		...
		getCSSAnimationSupport : function() {
			var animation = false, animationstring = 'animation', keyframeprefix = '', domPrefixes = 'Webkit Moz O ms Khtml'.split(' '), pfx = '', elm = this._container
					.get(0);
		...
		setStyles : function() {
			var requiredStyles = "<style>" + ".jp-invisible { visibility: hidden !important; } " + ".jp-hidden { display: none !important; }"
					+ "</style>";
		...
		cacheNavElements : function(holder, index) {
			...
			this._nav[index].permPages = this._nav[index].pages.slice(0, this.options.startRange).add(
					this._nav[index].pages.slice(this._numPages - this.options.endRange, this._numPages));
		...
		getInterval : function(page) {
			...
			end = page > neHalf ? Math.min(page + neHalf - (this.options.midRange % 2 > 0 ? 1 : 0), this._numPages) : Math.min(this.options.midRange,
					this._numPages);
			return {
				start : start,
				end : end
